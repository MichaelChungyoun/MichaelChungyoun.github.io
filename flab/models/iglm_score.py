...
from iglm_model import iglm_ppl

MODEL_NAME = "iglm"


def safe_score(seq, chain_type):
    if pd.isna(seq) or str(seq).strip() == "":
        return np.nan
    try:
        return iglm_ppl(seq, chain_type)
    except Exception:
        return np.nan
...
def score_dataframe(df, heavy_col, light_col):
    def score_row(row):
        heavy_score = safe_score(row.get(heavy_col, np.nan), "[HEAVY]")
        light_score = safe_score(row.get(light_col, np.nan), "[LIGHT]")
        return np.nanmean([heavy_score, light_score])

    df[MODEL_NAME] = df.apply(score_row, axis=1)
    return df
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Score antibody sequences with IgLM perplexity.")
