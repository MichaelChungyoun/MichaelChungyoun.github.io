...
def score_dataframe(df, heavy_col, light_col):
    def score_row(row):
        ...
        if pd.isna(heavy_score) and pd.isna(light_score):
            total = np.nan
        elif pd.isna(heavy_score):
            total = light_score
        elif pd.isna(light_score):
            total = heavy_score
        else:
            total = heavy_score + light_score
        return pd.Series({MODEL_NAME: total})
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Score antibody sequences with Levenshtein distance (LDv + LDj, heavy + light).")
